...
// sftpgoClient may also be nil (see caseService.sftpgo's own doc comment) —
// every environment without SFTPGO_BASE_URL configured keeps working for
// every route except the SFTPGo-backed attachment/inline-image ones.
func NewCaseService(repo repository.CaseRepository, userRepo repository.UserRepository, publisher EventPublisherService, sftpgoClient SFTPGoFileClient) CaseService {
	return &caseService{repo: repo, userRepo: userRepo, publisher: publisher, sftpgo: sftpgoClient}
}
...
func (s *caseService) CreateCaseAttachment(ctx context.Context, req domain.CreateAttachmentRequest) (domain.CreateAttachmentResponse, error) {
	...
	caseView, err := s.repo.GetCaseByID(ctx, req.ReferenceID)
	if err != nil {
		return domain.CreateAttachmentResponse{}, err
	}
